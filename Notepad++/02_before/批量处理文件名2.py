...
class BatchRename():
    ...
    '''
    def __init__(self):
        self.path = r'C:\Users\Administrator\Desktop\test'  #表示需要命名处理的文件夹
    ...
    def rename(self):
        ...
        for item in filelist:
            if item.endswith('.PNG'):  #初始的图片的格式为jpg格式的（或者源文件是png格式及其他格式，后面的转换格式就可以调整为自己需要的格式即可）
                ...
                dst = os.path.join(os.path.abspath(self.path), '1000' + format(str(i), '0>3s') + '.jpg')    #这种情况下的命名格式为0000000.jpg形式，可以自主定义想要的格式
                try:
                    os.rename(src, dst)
                    ...
                    continue
        print('total %d to rename & converted %d xmls' % (total_num, i))
